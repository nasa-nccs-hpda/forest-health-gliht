...
if not os.path.isfile(os.path.join(cfg.OUTPUT_DIR, "model_final.pth")): #don't rerun training unless I cleared the old one
...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.63   # set a custom testing threshold
predictor = DefaultPredictor(cfg)
...
for d in dataset_dicts:#random.sample(dataset_dicts, 3):    
    with CodeTimer('predict ' + os.path.basename(d["file_name"])):
